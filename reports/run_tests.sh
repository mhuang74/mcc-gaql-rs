...
run_test() {
    ...
    # Extract query (between SELECT and the first ══ line)
    query=$(echo "$output" | awk '/^SELECT/,/^═/ {print}' | sed '$d')
    
    # Extract explanation (between RAG SELECTION EXPLANATION and Total Generation Time)
    explanation=$(echo "$output" | awk '/RAG SELECTION EXPLANATION/{found=1; next} /^Total Generation Time/{found=0} found{print}')
    
    # Remove trailing empty lines
    query=$(echo "$query" | sed '/^$/d')
    explanation=$(echo "$explanation" | sed '/./,$\!d')
    
    # Create JSON, handling newlines properly
    json_output=$(cat <<EOF
{
...
EOF
    ...
    echo "$json_output" > "$RESULT_DIR/${entry_name}.json"

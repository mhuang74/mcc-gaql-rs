...
async fn test_cache_save_and_load_roundtrip() {
    ...
    assert_eq!(loaded.api_version, GOOGLEADS_API_VERSION);
...
async fn test_load_or_scrape_rescapes_when_stale() {
    ...
    responses.insert(format!("/{GOOGLEADS_API_VERSION}/campaign"), (200, campaign_html()));
    let base_url = start_mock_server(responses, 5).await;

    let result =
        ScrapedDocs::scrape_all_with_base_url(&["campaign".to_string()], GOOGLEADS_API_VERSION, 0, &base_url)
            .await
            .unwrap();
...
async fn test_scrape_all_with_mock_server_extracts_fields() {
    ...
    assert_eq!(result.api_version, GOOGLEADS_API_VERSION);
    assert_eq!(
...
async fn test_scrape_all_handles_too_small_page_gracefully() {
    ...
    let mut responses = HashMap::new();
    responses.insert(format!("/{GOOGLEADS_API_VERSION}/campaign"), (200, tiny_page_html()));
...
async fn test_scrape_all_handles_large_unrelated_page_gracefully() {
    ...
    let mut responses = HashMap::new();
    responses.insert(format!("/{GOOGLEADS_API_VERSION}/campaign"), (200, large_unrelated_html()));
...
async fn test_scrape_all_processes_multiple_resources() {
    ...
    responses.insert(format!("/{GOOGLEADS_API_VERSION}/ad_group"), (200, unqualified_id_html()));
